...
const { validateCoupon, markCouponUsed } = require('../utils/couponValidator');
...
const demoPaymentsEnabled = getEnv('PAYMENT_DEMO', 'true') === 'true';
...
const getBookingVehicleName = (booking) => {
  ...
  return [vehicle.make, vehicle.model, vehicle.year].filter(Boolean).join(' ') || 'Vehicle Rental';
};

const getBookingPaymentSummary = (booking) => {
  const totalAmount = Number(booking.pricing?.totalAmount ?? booking.totalAmount ?? 0);
...
const prepareAmountByMode = (booking, paymentMode) => {
  ...
  if (paymentMode === 'advance' && paidAmount === 0) {
    amount = round2(Number(booking.pricing?.advanceAmount || (totalAmount * advancePercentConfig) / 100));
    if (amount <= 0) {
      amount = dueAmount;
      mode = 'full';
    }
...
const callSslCommerzInit = async ({ booking, transaction, customer }) => {
  const payload = new URLSearchParams({
    ...
    shipping_method: 'NO',
    product_name: getBookingVehicleName(booking),
...
const buildDemoGatewayUrl = (transactionId, message) => {
  const params = new URLSearchParams({
    status: 'success',
    transactionId,
    message: message || 'Demo payment completed',
  });
  return `${frontendBase}/payment-result?${params.toString()}`;
};

const notifyPaymentSuccess = async (booking, transaction, settledAmount) => {
  try {
...
const updateBookingAfterPayment = async (booking, amount, isSuccess) => {
  if (!isSuccess) {
    booking.paymentStatus = Number(booking.paidAmount || booking.pricing?.advancePaid || 0) > 0 ? 'partial_paid' : 'failed';
    await booking.save();
    return booking;
  }

  booking.paidAmount = round2(Number(booking.paidAmount || booking.pricing?.advancePaid || 0) + Number(amount || 0));
  booking.pricing = booking.pricing || {};
  booking.pricing.advancePaid = booking.paidAmount;
  booking.pricing.balanceDue = round2(Math.max(0, Number(booking.pricing.totalAmount || booking.totalAmount || 0) - booking.paidAmount));
  booking.totalAmount = booking.pricing.totalAmount || booking.totalAmount || 0;

  if (booking.paidAmount >= Number(booking.pricing.totalAmount || booking.totalAmount || 0)) {
...
exports.getCustomerBookingsForPayment = async (req, res) => {
  try {
    ...
    const payload = bookings.map((booking) => {
      ...
      return {
        _id: booking._id,
        vehicleName: getBookingVehicleName(booking),
...
exports.initiatePayment = async (req, res) => {
  try {
    ...
    const booking = await Booking.findOne({ _id: bookingId, customer: req.user._id }).populate('vehicle');
    if (!booking) {
    ...
    if (couponCode) {
      const orderAmount = Number(booking.pricing?.baseRate || 0) + Number(booking.pricing?.driverFee || 0) + Number(booking.pricing?.serviceFee || 0);
      ...
      booking.pricing = booking.pricing || {};
      booking.pricing.couponCode = couponResult.coupon.code;
      booking.pricing.couponDiscount = couponResult.discount;
      booking.pricing.totalAmount = round2(orderAmount - couponResult.discount);
      booking.pricing.advanceAmount = round2(booking.pricing.totalAmount * (advancePercentConfig / 100));
      booking.pricing.advancePaid = 0;
      booking.pricing.balanceDue = round2(Math.max(0, booking.pricing.totalAmount - booking.pricing.advancePaid));
      booking.totalAmount = booking.pricing.totalAmount;
    ...
    if (!storeId || !storePassword) {
      ...
      await updateBookingAfterPayment(booking, paymentPlan.amount, true);

      if (booking.pricing?.couponCode) {
        await markCouponUsed(booking.pricing.couponCode, req.user._id, booking._id);
      }

      await notifyPaymentSuccess(booking, transaction, paymentPlan.amount);
      ...
      return res.status(201).json({
        ...
        gatewayUrl: buildDemoGatewayUrl(transaction.transactionId, 'Demo payment completed'),
        ...
        booking: {
          ...
          balanceDue: booking.pricing?.balanceDue,
  ...
  if (nextStatus === 'success') {
    ...
    transaction.receipt = {
      ...
      details: {
        ...
        vehicleName: getBookingVehicleName(transaction.booking),
    ...
    if (booking.pricing?.couponCode) {
      await markCouponUsed(booking.pricing.couponCode, transaction.customer, booking._id);
    }

    await notifyPaymentSuccess(booking, transaction, settledAmount);
