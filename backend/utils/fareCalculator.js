...
function calculateFares(distanceKm, durationMinutes, vehicles, rentalMode = 'with_driver') {
  ...
  const vehicleOffers = vehicles.map((vehicle) => {
    ...
    const supportsDriver = ['with_driver', 'both'].includes(vehicle.rentalMode);
    const bookingMode = rentalMode === 'with_driver' ? 'request' : 'instant';
    ...
    return {
      ...
      rentalMode,
      bookingMode,
      driverAvailability: supportsDriver ? 'available' : 'not_available',
      cancellationPolicy: 'Free cancellation up to 24h before pickup; 50% refund within 24h; no-show is non-refundable.',
      estimatedFare,
      fareBreakdown: {
        baseFare,
        driverFee,
