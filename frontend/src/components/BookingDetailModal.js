...
import { assignDriverToBooking, getVendorDrivers } from '../services/driverService';
...
const BookingDetailModal = ({ booking, onClose, onStatusUpdate, onPaymentUpdate }) => {
  ...
  useEffect(() => {
    const vendorId = booking.vendor?._id || booking.vendorId;
    if (!vendorId) {
      setVendorDrivers([]);
      return;
    }
    ...
    const loadDrivers = async () => {
      try {
        ...
        setVendorDrivers(Array.isArray(data) ? data : []);
      } catch (err) {
        console.error(err);
      }
    };
            ...
            <div className="tab-content">
              <div className="detail-section">
                ...
                <div className="action-form" style={{ marginBottom: '1rem' }}>
                    ...
                    <select value={selectedDriverId} onChange={(e) => setSelectedDriverId(e.target.value)}>
                      ...
                      {vendorDrivers.map((driver) => (
                        <option key={driver._id} value={driver._id}>
                          {driver.fullName} {driver.averageRating ? `(${driver.averageRating}/5)` : ''}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  >
                    {updating ? 'Assigning...' : 'Assign Selected Driver'}
                  </button>
                </div>
